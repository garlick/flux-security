...
The Flux Independent Minister of Privilege (IMP or ``flux-imp``) is
...
:linux:man7:`glob` pattern ``${sysconfdir}/etc/flux/imp/conf.d/*.toml``.
The configuration files in this directory are security sensitive, and such,
